...
(define-map minted-in-period principal uint)
...
(define-read-only (get-remaining-mint-allowance (user principal))
  (let
    (
      ...
      (blocks-passed (- block-height last-mint))
    )
...
(define-public (mint (amount uint))
  (let
    ...
    ;; Check daily limit
    (asserts! 
      (<= (+ amount (default-to u0 (map-get? minted-in-period tx-sender))) max-mint-per-day)
    ...
    ;; Update tracking
    (map-set last-mint-block tx-sender block-height)
    (map-set minted-in-period tx-sender (+ amount (default-to u0 (map-get? minted-in-period tx-sender))))
